...
def generateModelInstance(model, prevClasses, newClasses, activationFlag = True):
    newModel = mvrf(n_classes = prevClasses + newClasses, height=imgHeight, width= imgWidth, addActivation = activationFlag)
    #model.summary()
    #newModel.summary()
...
def trainModelsIncrementally(numIterations):
    ...
    for i in range(numIterations):
        if i is 0: # semantic segmentation (no incremental training)
            model = mvrf(n_classes=newClasses[i] , height = imgHeight, width = imgWidth)
